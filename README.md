### Project Overview ###
The project is a PowerBI-like app for public health researchers and data scientists to create simple statistical models to investigate
the effects of enforcing restrictions on chosen variables. 

The app consists of 4 pages, each with a unique functionality:
# Dataset #
The dataset page consists of a CRUD functionality, which allows for creating, deleting and modifying tables through data insertion from csv. The user has a choice to perform operations of the original database (covid.db), which contains data on restrictions enforced in time, or a custom database (custom.db), which contains 2 examplary tables with data.
...
# Time Series #
The time series page allows the user to graph the number of restrictions in time and overlay it with any other variable, which has data allocated in one the databases to find patterns and correlation. It creates a linear regression model for the two variables and enables the user to a local LLM to comment on the results.

# Timeline #
The timeline page allows the users to see how restrictions have been enforced time on a timeline and allows the user to view the source for each restriction by clicking on it in the timeline.
...
# Record Modification #
The app does not allow the user to create, update and delete individual records; therefore, the only way to modify a table is by deleting it, creating it again and inserting a new csv file. Such implementaion massively simplifies the code; however, can make it difficult for users to fix errors in data. To improve the user experience a functionality can be added that allows the user to create, update and delete records in individual tables. 
